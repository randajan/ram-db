...
export class RowSync extends jet.types.Plex {
    ...
    static create(rows, vals, opt={ autoSave:true, resetOnError:true, saveError:true }) {
      const row = new RowSync(rows);
      if (jet.isMapable(vals)) { row.set(vals, opt); }
      return row;
    };
