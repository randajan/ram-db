...
export class Transactions {
    ...
    async execute(name, execution, opt={ stopOnError:false, throwError:true }) {
        const exe = async last=>{
            try { await last; } catch(e) {}
            ...
            if (!isErr) { 
                this.state = name;
                try { await execution(this.ticks++); } catch(e) { isErr = true; err = e; }
